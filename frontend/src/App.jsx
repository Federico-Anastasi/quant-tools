...
function App() {
  ...
  const fetchCandles = async () => {
    try {
      ...
      console.error('[App] Error fetching candles:', err)
  ...
  const fetchLOBDensity = async () => {
    try {
      console.log('[App] Fetching LOB density with price_bin:', priceBin)
      ...
      console.log('[App] LOB density response:', {
      ...
      console.error('[App] Error fetching LOB density:', err)
    }
  }

  const fetchOrderFlowZones = async () => {
    try {
      console.log('[App] Fetching order flow zones...')
      const response = await axios.get(`${API_URL}/api/order-flow-zones`, {
        params: { symbol: 'BTC' }
      })

      console.log('[App] Order flow zones response:', {
        timestamp: response.data.timestamp,
      ...
      console.error('[App] Error fetching order flow zones:', err)
    }
  }
  ...
  const handleScreenshot = async () => {
    console.log('[App] Screenshot requested for tab:', activeTab)
    ...
    try {
      ...
      if (!appContainer) {
        console.warn('[App] App container not found')
        return
      }
      ...
      console.log(`[App] Full page screenshot saved: ${filename}`)
    } catch (err) {
  ...
  const handleResetView = () => {
    console.log('[App] Reset view requested - User should double-click axis')
